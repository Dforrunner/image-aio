...
const SUPPORTED_INPUT_FORMATS = ["jpeg", "jpg", "png", "webp", "gif", "avif", "tiff", "tif", "svg", "heif"]
    ...
    const processedImages = await Promise.allSettled(
      files.map(async (file) => {
        try {
          ...
          const originalDataUrl = `data:image/${metadata.format};base64,${buffer.toString("base64")}`
          ...
          const percentageSaved = Math.round(((originalSize - processedSize) / originalSize) * 100)
          ...
          return {
            ...
            originalDataUrl,
            processedDataUrl: `data:image/${settings.format};base64,${processedBuffer.toString("base64")}`,
            percentageSaved,
